...
class Cart extends React.Component {
  ...
  render() {
    const { cart, loaded, saveState } = this.state;
    let ready = true;
    cart.forEach((photo) => {
                ...
                <>
                        ...
                        <>
                          ...
                          <div className="cart-line">
                            ...
                            <div className="cart-item-options">
                              ...
                              <label className="min-margin" htmlFor="size">
                                Choose a size:
                              </label>
                              <select
                                className="min-margin cart-select"
                                id="size"
                                name="size"
                                onChange={(e) => {
                                  this.handleChage(
                              ...
                              >
                                ...
                                {photo.sizes.map((size) => {
                                  return (
                                    <option
                                      selected={photo.size === size.info}
                                      key={size.info}
                                      value={JSON.stringify({
                                        size: size.info,
                                        price: size.price,
                                      })}
                                    >{`${size.info} ($${size.price})`}</option>
                                  );
                                })}
                              </select>
                              <button
                  ...
                  <div className="menu-buttons">
                    ...
                    {ready ? (
                      ...
                      <h3 className="badpass">
                        Please select a size & finish for each item to check out
                      </h3>
                    )}
                  </div>
                </>
