...
def accounts():
    ...
    with pennington_photo.app.app_context():
        connection = pennington_photo.model.get_db()

        # check if target is unspecified or blank
        target = pennington_photo.model.get_target()
...
def do_login(uname, pword):
    ...
    logname = pennington_photo.model.check_authorization(uname, pword)
    if not logname:
        abort(403)
...
def do_update_password(connection, info):
    ...
    if len(salt) > 1:
        salt = salt[1]
        pw_str = pennington_photo.model.encrypt(salt, info['old'])
    else:
        pw_str = info['old']
...
@pennington_photo.app.route('/accounts/logout/', methods=['POST'])
def logout():
